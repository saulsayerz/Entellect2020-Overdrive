...
public class Bot {
    ...
    public Command run(GameState gameState) {
        ...
        if (isCT && blocks.subList(0, min(blocks.size(), myCar.speed)).contains(Terrain.WALL)) {
            // Kasus 1
            if (lanepos == 1) {
            ...
            if (lanepos == 2 || lanepos == 3) {
                ...
                if (rBlocks.subList(0, min(blocks.size(), myCar.speed)).contains(Terrain.WALL)
                        && lBlocks.subList(0, min(blocks.size(), myCar.speed)).contains(Terrain.WALL)) {
                    if (hasPowerUp(PowerUps.LIZARD, myCar.powerups)) {
                        return LIZARD;
        ...
        if (blocks.subList(0, min(blocks.size(), myCar.speed)).contains(Terrain.WALL)) {
            ...
            if (myCar.position.lane == 4) {
                if (!lBlocks.subList(0, min(blocks.size(), myCar.speed)).contains(Terrain.WALL) && !isCTLeft) {
        ...
        if (blocks.subList(0, min(blocks.size(), myCar.speed)).contains(Terrain.MUD)
            ...
            if (lanepos == 1) { // kasus dia di paling atas, hanya mungkin turn right
                if (rBlocks.subList(0, min(blocks.size(), myCar.speed)).contains(Terrain.MUD)
                        || rBlocks.subList(0, min(blocks.size(),
                                myCar.speed)).contains(Terrain.OIL_SPILL)
            ...
            if (lanepos == 2 || lanepos == 3) { // ini bisa ke kiri bisa ke kanan
                ...
                if (kananaman && kiriaman) { // cek mana yang ada powernya
                    ...
                    if (rBlocks.subList(0, min(blocks.size(),
                            myCar.speed)).contains(Terrain.OIL_POWER)
                            || rBlocks.subList(0, min(blocks.size(),
                                    myCar.speed)).contains(Terrain.LIZARD)
                            || rBlocks.subList(0, min(blocks.size(), myCar.speed)).contains(Terrain.EMP)
                            || rBlocks.subList(0, min(blocks.size(),
                                    myCar.speed)).contains(Terrain.BOOST)
                            || rBlocks.subList(0, min(blocks.size(),
                                    myCar.speed)).contains(Terrain.TWEET)) {
                        kananadapower = true;
                    }
                    if (lBlocks.subList(0, min(blocks.size(),
                            myCar.speed)).contains(Terrain.OIL_POWER)
